...
$id=$_GET['ff'];
$work=$conn->query("SELECT * FROM `worker_attendance` where id=$id");
...
if(isset($_POST['status'])){
 ...
 $bb=$conn->query("UPDATE `worker_attendance` SET `wrokerID` = ' $wrokerID', `status` = '$status', `date` = '$date', `monthID` = ' $month', `over_time` = '$time' WHERE `worker_attendance`.`id` = $id;)");
 //  if ($bb) {
 //   echo "inserted";
 // }
 // else{
...
<tr/>
            ...
            <tr/>
                      <th>Status<th/>
                <td>
          <input type="radio"  name="status" value= "present" <?php if ($haw['status']==='present'){echo "checked"; }?>><span>present </span>
                 <input type="radio"  name="status" value= "absent"<?php if ($haw['status']==='absent'){echo "checked"; }?> ><span> absent</span>
